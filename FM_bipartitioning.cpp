...
bool LoopPass(int pass_num)
{
    ...
    {   
        ...
        if( (cutsize < min_cutsize && balance/best_balance <1.2 )|| //the hill climp ability
...
int main()
{
    ...
    for (int i=0;i<cells.size();i++)
    {
        cout << cells[i]->cell_name <<"  " <<cells[i]->partition << "  "<<cells[i]->cell_size <<endl;
    }*/
    computeCutsize();
    computeGain();
    cout << "\n"<<endl;
    ...
    while(pass_num<=9 && LoopPass(pass_num))
    {
        pass_num ++;
    }
    recallbestsolution();
